...
<input type="hidden" name="p_id" value="$_REQUEST['p_id']">
	...
	<tr>
		<th colspan="6" class="td"><h1>View Order </h1></th>
	...
	<?php
		...
		session_start();
		$que="select * from bill1 where b1_r_id='".$_SESSION['r_id']."'&&b1_status='".(0)."'order by b1_id";
		$res=mysql_query($que,$con);
		while($row=mysql_fetch_array($res))
		{
			 $que1="select * from bill2 where b2_b1_id='".$row['b_id']."' ";
			  $res1=mysql_query($que1,$con);
			 
			while($row1=mysql_fetch_array($res1))
			{
				$que2="select * from product where p_id=".$row1['p_id'];
				...
				if($row2=mysql_fetch_array($res2))
				{
					...
					echo"<td>".$tot=$row1['p_quantity']*$row2['p_price']."</td>";
					$bill1+=$tot;
					echo"</tr>";
				}
			}
	...
	<tr>
		...
		<th colspan="2"><h2><?php echo $bill;?></h2></th>
	</tr>
